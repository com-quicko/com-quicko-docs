...
<Steps>
  ...
  <Step title="Acquiring Access and Refresh Tokens">
    ...
    The Host calls the [OAuth Authorize API](/connect/api-reference/authentication/oauth-authorize) with:
...
1. **Redirect** - Host App redirects user to Quicko OAuth with `api_key` and `redirect_url`
