---
title: "Quickstart"
description: "Set up a Quicko account, register your app, and provision credentials for consent-based access to your users' data."
---

This quickstart walks through onboarding to **Quicko Connect** — the OAuth-based workflow that lets your application access your users' data on Quicko after they grant consent.
...
<Warning>
  Newly created apps are in **Draft** status and cannot be used to make live OAuth requests. After submitting the form, notify the Quicko team to review and approve the app.
</Warning>
...
- Use credentials prefixed with `key_test_…` / `secret_test_…` against the test environment, and `key_live_…` / `secret_live_…` against production.
